...
namespace ps2recomp
{
    ...
    std::vector<Function> ElfParser::extractFunctions() const
    {
        ...
        // Explicit Ghidra ranges take precedence over every inferred or symbol range.
        std::unordered_map<uint32_t, uint32_t> authoritativeEndByStart;
        authoritativeEndByStart.reserve(m_symbols.size() + m_ghidraMapEndByStart.size());
        for (const auto &[start, end] : m_ghidraMapEndByStart)
        {
            authoritativeEndByStart.emplace(start, end);
        }

        // Symbol table sizes are authoritative when no configured Ghidra range exists.
        for (const auto &symbol : m_symbols)
        {
            ...
            if (m_ghidraMapEndByStart.contains(symbol.address))
            {
                continue;
            }
        ...
        // Named debug functions with explicit bounds are authoritative too. Ghidra
        // functions were inserted above, independently of whether their names are
        // auto-generated.
        for (const auto &extra : m_extraFunctions)
        {
            ...
            if (m_ghidraMapEndByStart.contains(extra.start))
            {
                continue;
            }
        ...
        auto addOrMerge = [&](const Function &newFunction)
        {
            ...
            if (insideAuthoritativeRange && !hasOwnAuthoritativeRange)
            {
                return;
            }
    ...
    bool ElfParser::loadGhidraFunctionMap(const std::string &mapPath)
    {
        ...
        if (!mapFunctions.empty())
        {
            std::sort(mapFunctions.begin(), mapFunctions.end(),
                      [](const Function &a, const Function &b)
                      {
                          if (a.start != b.start)
                          {
            ...
            mapFunctions.erase(
                std::unique(mapFunctions.begin(), mapFunctions.end(),
                            [](const Function &a, const Function &b)
                            { return a.start == b.start; }),
                mapFunctions.end());

            std::unordered_set<uint32_t> mapStarts;
            std::unordered_map<uint32_t, uint32_t> mapEndByStart;
            mapStarts.reserve(mapFunctions.size());
            mapEndByStart.reserve(mapFunctions.size());
            for (const auto &func : mapFunctions)
            {
                mapStarts.insert(func.start);
                mapEndByStart.emplace(func.start, func.end);
            }

            m_hasLoadedGhidraMap = true;
            m_ghidraMapStarts = std::move(mapStarts);
            m_ghidraMapEndByStart = std::move(mapEndByStart);

            // A configured Ghidra map supersedes anonymous JAL fallback and
            // anonymous debug discoveries. Remove those before appending map
            // functions so a longer inferred range at the same start cannot win.
            ...
            if (m_reporter)
            {
                m_reporter->info("ghidra-map", "Loaded " + std::to_string(m_ghidraMapStarts.size()) + " functions from Ghidra map");
            }
